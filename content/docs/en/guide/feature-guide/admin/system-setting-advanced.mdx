...
- **type**: Channel identifier, used for backend routing and order placement.
  ...
  - For detailed logic, see the backend controller [controller/topup.go](https://github.com/AItoken-cloud/tokenjump-docs-v1/blob/main/controller/topup.go).
...
- For detailed implementation logic, see the backend controller [controller/topup.go](https://github.com/AItoken-cloud/tokenjump-docs-v1/blob/main/controller/topup.go).
...
Ratio settings are the core configuration of the TokenJump billing system. By setting different Ratios, you can flexibly control the billing standards for various models and user groups.

### Ratio System Overview

TokenJump uses a three-tier Ratio system to calculate user Quota consumption:
...
In the TokenJump system, Ratios are key parameters for calculating Quota consumption. Quota is the internal billing unit of the system, and all API calls are ultimately converted into Quota points for deduction.
...
TokenJump employs a dual billing mechanism of pre-consumption and post-consumption:
