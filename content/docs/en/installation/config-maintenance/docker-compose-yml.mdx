...
This document details the Docker Compose configuration options for TokenJump, applicable to various deployment scenarios.

## Basic Configuration Structure

The Docker Compose configuration file `docker-compose.yml` defines how the TokenJump service and its dependent services (such as MySQL, Redis) are deployed.
...
For testing purposes, you can use the following simplified version, which only includes the TokenJump service itself:
...
### TokenJump Service Configuration
...
| `environment`    | Environment variable settings, used to configure TokenJump behavior              |
...
TokenJump supports various environment variable configurations. Here are some commonly used ones:
